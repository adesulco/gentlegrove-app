...
export default function RoutineGame({ phase, onComplete, settings, companionEmoji, companionName }) {
  ...
  useEffect(() => { setShuffled(shuffle(routine.steps)); setPlaced([]); }, [phase, routine]);
  ...
  return (
    <div style={{ padding: 20 }}>
      ...
      {feedback === "tryAgain" && (
        <div style={{ textAlign: "center", padding: 14, background: "#FFF8E8", borderRadius: 16, marginTop: 18, animation: "shake 0.4s ease" }}>
          <span style={{ fontSize: 32 }}>{companionEmoji}</span>
          <div style={{ color: "#B8860B", fontWeight: 600, fontSize: 18 }}>Not that one yet! Think about what comes first.</div>
        </div>
      )}
      {feedback === "complete" && (
        <div style={{ textAlign: "center", padding: 18, background: "#E8F8E8", borderRadius: 16, marginTop: 18, animation: "popIn 0.4s ease" }}>
          <span style={{ fontSize: 32 }}>{companionEmoji}</span>
          <div style={{ color: "#2E8B57", fontWeight: 700, fontSize: 18 }}>Perfect routine! {companionName} is so happy!</div>
        </div>
      )}
    </div>
  );
