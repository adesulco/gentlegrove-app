...
import { getRandomQuestions, generateVocabQuestions } from '../../utils/questionEngine';
...
function CommunicationWordGame({ vocabSet, onComplete, settings, companionEmoji, companionName, phase }) {
  const vocab = useMemo(() => generateVocabQuestions(vocabSet, 6), [vocabSet]);
  ...
  const bgColor = settings.theme === "warm" ? "#F8FFF0" : settings.thme === "neutral" ? "#F8F8F6" : "#F0FFF8";
    ...
    <div style={{ padding: 20 }}>
      ...
      {feedback === "wrong" && (
        <div style={{ textAlign: "center", padding: 18, background: "#FFF8E8", borderRadius: 16, marginTop: 18, animation: "gentlePulse 0.4s ease" }}>
          <span style={{ fontSize: 32 }}>{companionEmoji}</span>
          <div style={{ color: "#B8860B", fontWeight: 600, fontSize: 18, marginTop: 4 }}>Good try! Let's learn together.</div>
        </div>
      )}
      {feedback === "reveal" && (
...
function SentenceGame({ onComplete, settings, companionEmoji, companionName }) {
  const questions = useMemo(() => getRandomQuestions(VOCAB_NAVIGATORS, 6), []);
  const [current, setCurrent] = useState(0);
  const [feedback, setFeedback] = useState(null);
  const [correctCount, setCorrectCount] = useState(0);
  const [xp, setXp] = useState(0);
    ...
    <div style={{ padding: 20 }}>
      ...
      {feedback === "reveal" && (
        <div style={{ marginTop: 18 }}>
          ...
          {q.teach && <div style={{ textAlign: "center", color: "#666", fontSize: 14, marginTop: 8, fontStyle: "italic", padding: "0 20px" }}>{w.teach}</div>}
        </div>
      )}
    </div>
  );
...
export default function CommunicationGame({ phase, onComplete, settings, companionEmoji, companionName }) {
  ...
  const vocab = vocabSets[phase] || VOCAB_SEEDLENGS;
