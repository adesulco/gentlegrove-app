...
import { generateEmotionQuestions } from '../../utils/questionEngine';
...
export default function EmotionGame({ phase, onComplete, settings, companionEmoji, companionName }) {
  const scenarios = useMemo(() => {
    ...
    const pool = scenarioSets[phase] || SCENARIOS_SEEDLINGS;
    return generateEmotionQuestions(pool, EMOTIONS, 6);
  }, [phase]);
  ...
  return (
    <div style={{ padding: 20 }}>
      ...
      {feedback === "wrong" && (
        <div style={{ textAlign: "center", padding: 18, background: "#FFF8E8", borderRadius: 16, animation: "gentlePulse 0.4s ease" }}>
          <span style={{ fontSize: 36 }}>{companionEmoji}</span>
          <div style={{ color: "#B8860B", fontWeight: 600, fontSize: 18, marginTop: 4 }}>That's okay! Let's learn together.</div>
