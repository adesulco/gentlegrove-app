...
export default function PatternGame({ phase, onComplete, settings, companionEmoji, companionName }) {
  ...
  const patterns = useMemo(() => getRandomQuestions(patternSets[phase] || PATTERNS_SEEDLINGS, 5), [phase]);
    ...
    <div style={{ padding: 20 }}>
      ...
      {feedback === "correct" && (
        <div style={{ textAlign: "center", padding: 18, background: "#E8F8E8", borderRadius: 16, marginTop: 18, animation: "popIn 0.3s ease" }}>
          ...
          <div style={{ color: "#4A7C59", fontWeight: 700, fontSize: 18, marginTop: 4 }}>Great pattern spotting! {companionName} is amazed!</div>
        </div>
      )}
      {feedback === "wrong" && (
        <div style={{ textAlign: "center", padding: 18, background: "#FFF8E8", borderRadius: 16, marginTop: 18, animation: "gentlePulse 0.4s ease" }}>
          <span style={{ fontSize: 32 }}>{companionEmoji}</span>
          <div style={{ color: "#B8860B", fontWeight: 600, fontSize: 18, marginTop: 4 }}>Good try! Let's look at the pattern together.</div>
