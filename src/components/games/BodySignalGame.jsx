...
function BodySignalSeedlings({ onComplete, settings, companionEmoji, companionName }) {
  const items = BODY_SIGNALS_SEEDLINGS;
    ...
    <div style={{ padding: 20 }}>
      ...
      {feedback === "correct" && (
        <div style={{ textAlign: "center", padding: 18, background: "#E8F8E8", borderRadius: 16, marginTop: 18, animation: "popIn 0.3s ease" }}>
          ...
          <div style={{ color: "#2E8B57", fontWeight: 700, fontSize: 18 }}>You know your body signals! {companionName} is impressed!</div>
        </div>
      )}
      {feedback === "wrong" && (
        <div style={{ textAlign: "center", padding: 18, background: "#FFF8E8", borderRadius: 16, marginTop: 18, animation: "shake 0.4s ease" }}>
          <span style={{ fontSize: 32 }}>{companionEmoji}</span>
          <div style={{ color: "#B8860B", fontWeight: 600, fontSize: 18 }}>Let's see the answer!</div>
        </div>
      )}
      {feedback === "reveal" && (
...
function BodySignalExplorers({ onComplete, settings, companionEmoji, companionName }) {
  const items = BODY_SIGNALS_EXPLORERS;
  const [current, setCurrent] = useState(0);
  const [feedback, setFeedback] = useState(null);
  const [correctCount, setCorrectCount] = useState(0);
  const [xp, setXp] = useState(0);
  ...
  return (
    <div style={{ padding: 20 }}>
      ...
      {feedback === "correct" && (
        <div style={{ textAlign: "center", padding: 18, background: "#E8F8E8", borderRadius: 16, marginTop: 18, animation: "popIn 0.3s ease" }}>
          <span style={{ fontSize: 32 }}>{companionEmoji}</span>
          <div style={{ color: "#2E8B57", fontWeight: 700, fontSize: 18 }}>Body signal expert! {companionName} is learning too!</div>
        </div>
      )}
      {feedback === "wrong" && (
        <div style={{ textAlign: "center", padding: 18, background: "#FFF8E8", borderRadius: 16, marginTop: 18, animation: "shake 0.4s ease" }}>
          <span style={{ fontSize: 32 }}>{companionEmoji}</span>
          <div style={{ color: "#B8860B", fontWeight: 600, fontSize: 18 }}>Not quite -- let us check!</div>
        </div>
      )}
      {feedback === "reveal" && (
        <div style={{ marginTop: 18 }}>
  ...
  return (
    <div style={{ padding: 20 }}>
      ...
      {feedback === "correct" && (
        <div style={{ textAlign: "center", padding: 18, background: "#E8F8E8", borderRadius: 16, marginTop: 18, animation: "popIn 0.3s ease" }}>
          <span style={{ fontSize: 32 }}>{companionEmoji}</span>
          <div style={{ color: "#2E8B57", fontWeight: 700, fontSize: 18 }}>Great strategy! {companionName} feels calmer already!</div>
        </div>
      )}
      {feedback === "wrong" && (
        <div style={{ textAlign: "center", padding: 18, background: "#FFF8E8", borderRadius: 16, marginTop: 18, animation: "shake 0.4s ease" }}>
          <span style={{ fontSize: 32 }}>{companionEmoji}</span>
          <div style={{ color: "#B8860B", fontWeight: 600, fontSize: 18 }}>Let's see a better approach!</div>
        </div>
      )}
      {feedback === "reveal" && (
        <div style={{ marginTop: 18 }}>
...
export default function BodySignalGame({ phase, onComplete, settings, companionEmoji, companionName }) {
  ...
  return <BodySignalNavigators onComplete={onComplete} settings={settings} companionEmoji={companionEmoji} companionName={companionName} />;
}
