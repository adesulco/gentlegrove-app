    ...
    <div style={{ padding: 20 }}>
      ...
      {feedback === "wrong" && (
        <div style={{ textAlign: "center", padding: 18, background: "#FFF8E8", borderRadius: 16, marginTop: 18, animation: "shake 0.4s ease" }}>
          <span style={{ fontSize: 32 }}>{companionEmoji}</span>
          <div style={{ color: "#B8860B", fontWeight: 600, fontSize: 18, marginTop: 4 }}>Let us see the right letter!</div>
        </div>
      )}
      {feedback === "reveal" && (
...
export default function AlphabetGame({ phase, onComplete, settings, companionEmoji, companionName }) {
  ...
  if (phase === "explorers") return <AlphabetExplorersNav items={ALPHA_EXPLORERS} onComplete={onComplete} settings={settings} companionEmoji={companionEmoji} companionName={companionName} />;
  return <AlphabetExplorersNav items={ALPHA_NAVIGATORS} onComplete={onComplete} settings={settings} companionEmoji={companionEmoji} companionName={companionName} />;
}
